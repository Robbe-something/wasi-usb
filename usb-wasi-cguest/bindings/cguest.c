...
void component_usb_usb_hotplug_list_tuple2_event_info_free(component_usb_usb_hotplug_list_tuple2_event_info_t *ptr) {
  size_t list_len = ptr->len;
  if (list_len > 0) {
    component_usb_usb_hotplug_tuple2_event_info_t *list_ptr = ptr->ptr;
    for (size_t i = 0; i < list_len; i++) {
    }
    free(list_ptr);
  }
...
void component_usb_usb_hotplug_poll_events(component_usb_usb_hotplug_list_tuple2_event_info_t *ret) {
  __attribute__((__aligned__(sizeof(void*))))
  uint8_t ret_area[(2*sizeof(void*))];
  uint8_t *ptr = (uint8_t *) &ret_area;
  __wasm_import_component_usb_usb_hotplug_poll_events(ptr);
  *ret = (component_usb_usb_hotplug_list_tuple2_event_info_t) { (component_usb_usb_hotplug_tuple2_event_info_t*)(*((uint8_t **) (ptr + 0))), (*((size_t*) (ptr + sizeof(void*)))) };
}
